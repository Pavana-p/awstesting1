...
You can deploy the ROOT.war archive that build.sh generates to a AWS Elastic Beanstalk web server environment running the Tomcat 8 platform.
...
The script compiles the project's classes, packs the necessary files into the web archive, and then attempts to copy the WAR file to ``/Library/Tomcat`` for local testing. If you installed Tomcat to another location, change the path in ``build.sh``:
